...
def _get_metric_timespan(helper):
    check_point_key = "%s_last_metric_date" % helper.get_input_stanza_names()
...
def _index_resource_metrics(helper, ew, access_token, resource_obj, metric_timespan, preferred_time_aggregation, requested_metric_statistics):
    ...
    for metric_list in metric_list_preferred:
        metric_url = "https://management.azure.com%s/providers/microsoft.insights/metrics?api-version=2018-01-01&timespan=%s&interval=%s&aggregation=%s&metricnames=%s" % \
    ...
    for metric_list in metric_list_alternate:
        metric_url = "https://management.azure.com%s/providers/microsoft.insights/metrics?api-version=2018-01-01&timespan=%s&aggregation=%s&metricnames=%s" % \
...
def index_metrics_for_resources(helper, ew, access_token, preferred_time_aggregation, metric_statistics, resources_to_query):
